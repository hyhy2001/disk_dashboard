...
import {
  ...
  deleteAccount, deleteDisk, deleteSpace, fetchAccounts, fetchAuthStatus,
...
function PwInput({ value, onChange, placeholder, autoFocus, autoComplete }: {
  ...
  return (
    <div className="relative">
      <Input type={show ? 'text' : 'password'} placeholder={placeholder} value={value} onChange={e => onChange(e.target.value)} autoFocus={autoFocus} autoComplete={autoComplete} className="pr-8" />
      <button type="button" onClick={() => setShow(s => !s)} className="absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors" tabIndex={-1}>
    ...
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent>
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction onClick={onConfirm} className="bg-destructive text-destructive-foreground hover:opacity-90">{action}</AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
  )
...
function LoginForm({ onLogin, rateLimit: rl }: { onLogin: (u: AuthInfo['user']) => void; rateLimit: AuthInfo['rateLimit'] }) {
  const [u, setU] = useState(''); const [p, setP] = useState('')
  const [err, setErr] = useState(''); const [busy, setBusy] = useState(false)
  const go = async (e: React.FormEvent) => {
    e.preventDefault(); setErr(''); setBusy(true)
    try { onLogin(await login(u, p)) } catch (e: any) { setErr(e.message) } finally { setBusy(false) }
  }
  ...
  return (
    <div className="flex min-h-[60vh] items-center justify-center">
      <Card className="w-[360px] animate-slide-up">
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><Shield className="size-4" />Admin Login</CardTitle>
          <p className="text-xs text-muted-foreground">Sign in to manage spaces, disks, and accounts.</p>
        </CardHeader>
        <CardContent>
          {rl.attempts > 0 && (
            <div className={`mb-3 rounded-sm px-3 py-2 text-xs ${rl.captcha ? 'bg-amber-400/10 text-amber-400' : 'bg-muted text-muted-foreground'}`}>
              {rl.captcha ? '⚠ Rate limit near — remaining attempts: ' : 'Remaining attempts: '}{remaining}
            </div>
          )}
          ...
          <form onSubmit={go} className="space-y-3">
            ...
            <Button type="submit" className="w-full" disabled={busy || !u || !p}>{busy ? 'Signing in…' : 'Sign in'}</Button>
          </form>
        </CardContent>
      </Card>
    </div>
...
function SetupForm({ onDone }: { onDone: (u: AuthInfo['user']) => void }) {
  ...
  const go = async (e: React.FormEvent) => {
    ...
    if (p.length < 10) { setErr('Password must be at least 10 characters'); return }
    setBusy(true)
    try { onDone(await setup(u, p)) } catch (e: any) { setErr(e.message) } finally { setBusy(false) }
  }
  return (
    <div className="flex min-h-[60vh] items-center justify-center">
      <Card className="w-[360px] animate-slide-up">
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><Shield className="size-4" />Initial Setup</CardTitle>
        ...
        <CardContent>
          ...
          <form onSubmit={go} className="space-y-3">
            ...
            <Button className="w-full" disabled={busy || !u || p.length < 10}>{busy ? 'Creating…' : 'Create Admin'}</Button>
          </form>
        </CardContent>
      </Card>
    </div>
...
function AccountsPanel() {
  ...
  const [show, setShow] = useState(false); const [nu, setNu] = useState(''); const [np, setNp] = useState(''); const [nr, setNr] = useState('admin')
  const [confirmId, setConfirmId] = useState<number | null>(null)
  const [confirmName, setConfirmName] = useState('')
  const [resetId, setResetId] = useState<number | null>(null); const [resetPw, setResetPw] = useState('')

  const load = useCallback(async () => { try { setList(await fetchAccounts()); setErr('') } catch { /* */ } }, [])
  useEffect(() => { void load() }, [load])

  const create = async (e: React.FormEvent) => {
    e.preventDefault(); setErr('')
    try { await createAccount(nu, np, nr); success('Account created', nu); setShow(false); setNu(''); setNp(''); await load() } catch (e: any) { setErr(e.message); failure('Create failed', e.message) }
  }
  const del = async (id: number) => {
    try { await deleteAccount(id); success('Account deleted'); await load() } catch (e: any) { failure('Delete failed', e.message) }
  }
  const doReset = async () => {
    if (!resetId) return
    try { await resetAccountPassword(resetId, resetPw); success('Password reset'); setResetId(null); setResetPw('') } catch (e: any) { failure('Reset failed', e.message) }
  }
  ...
  return (
    <Card>
      <CardHeader className="flex-row items-center justify-between space-y-0 py-3">
        ...
        <Button size="sm" onClick={() => setShow(true)}><Plus className="size-3.5 mr-1" />Add</Button>
      </CardHeader>
      {err && <div className="px-4 pb-2"><p className="rounded-sm bg-destructive/10 px-3 py-2 text-xs text-destructive">{err}</p></div>}
      <div className="overflow-auto">
        <table className="w-full text-sm">
          <thead><tr className="border-y border-border text-left text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
            <th className="py-2 pl-4">Username</th><th className="py-2">Role</th><th className="py-2">Created</th><th className="py-2 pr-4 text-right">Actions</th>
          </tr></thead>
          <tbody>
            {list.map(a => (
              <tr key={a.id} className="border-b border-border/50 hover:bg-muted/50 transition-colors">
                <td className="py-2 pl-4">{a.username}</td>
                <td className="py-2"><Badge variant={a.role === 'owner' ? 'default' : 'secondary'} className="text-[10px]">{a.role}</Badge></td>
                <td className="py-2 text-muted-foreground">{a.created_at.slice(0, 10)}</td>
                <td className="py-2 pr-4">
                  <div className="flex justify-end gap-1">
                    <Button variant="ghost" size="sm" onClick={() => { setResetId(a.id); setResetPw('') }}><Key className="size-3" /></Button>
                    {a.role !== 'owner' && <Button variant="ghost" size="sm" onClick={() => { setConfirmId(a.id); setConfirmName(a.username) }}><Trash2 className="size-3 text-destructive" /></Button>}
                  </div>
                </td>
              </tr>
            ))}
      ...
      <Dialog open={show} onOpenChange={setShow}>
        <DialogContent className="sm:max-w-[380px]">
          ...
          <form onSubmit={create} className="space-y-3">
            ...
            <select value={nr} onChange={e => setNr(e.target.value)} className="flex h-9 w-full rounded-sm border border-border bg-background px-3 text-sm">
              <option value="admin">admin</option><option value="owner">owner</option>
            </select>
            <Button type="submit" className="w-full" disabled={!nu || np.length < 10}>Create</Button>
          </form>
        </DialogContent>
      </Dialog>

      <ConfirmDialog open={confirmId !== null} onOpenChange={() => setConfirmId(null)} title="Delete Account" description={`Remove '${confirmName}'? This cannot be undone.`} action="Delete" onConfirm={() => { if (confirmId !== null) del(confirmId); setConfirmId(null) }} />
      ...
      <Dialog open={resetId !== null} onOpenChange={() => setResetId(null)}>
        <DialogContent className="sm:max-w-[380px]">
          ...
          <form onSubmit={e => { e.preventDefault(); doReset() }} className="space-y-3">
            ...
            <Button type="submit" className="w-full" disabled={resetPw.length < 10}>Reset</Button>
          </form>
        </DialogContent>
      </Dialog>
    </Card>
...
function SpacesPanel() {
  const [spaces, setSpaces] = useState<any[]>([]); const [err, setErr] = useState('')
  ...
  const [confirmSpaceId, setConfirmSpaceId] = useState<number | null>(null); const [confirmSpaceName, setConfirmSpaceName] = useState('')
  const [confirmDiskId, setConfirmDiskId] = useState<number | null>(null); const [confirmDiskName, setConfirmDiskName] = useState('')
  const [teamsDiskId, setTeamsDiskId] = useState<number | null>(null); const [teamsDiskName, setTeamsDiskName] = useState('')

  const load = useCallback(async () => { try { setSpaces(await fetchSpaces()); setErr('') } catch { /* */ } }, [])
  useEffect(() => { void load() }, [load])

  const addSpace = async (e: React.FormEvent) => { e.preventDefault()
    try { await createSpace(name); success('Space created', name); setModal(null); await load() } catch (e: any) { failure('Create failed', e.message) }
  }
  const renameSpace = async (e: React.FormEvent) => { e.preventDefault(); if (!modal?.id) return
    try { await updateSpace(modal.id, name); success('Space renamed', name); setModal(null); await load() } catch (e: any) { failure('Rename failed', e.message) }
  }
  const delSpace = async (id: number) => {
    try { await deleteSpace(id); success('Space deleted'); await load() } catch (e: any) { failure('Delete failed', e.message) }
  }
  const addDisk = async (e: React.FormEvent) => { e.preventDefault(); if (!modal?.spaceId) return
    try { await createDisk(modal.spaceId, name, path); success('Disk created', name); setModal(null); await load() } catch (e: any) { failure('Create failed', e.message) }
  }
  const editDisk = async (e: React.FormEvent) => { e.preventDefault(); if (!modal?.id) return
    try { await updateDisk(modal.id, { name, path }); success('Disk updated', name); setModal(null); await load() } catch (e: any) { failure('Update failed', e.message) }
  }
  const delDisk = async (id: number) => {
    try { await deleteDisk(id); success('Disk deleted'); await load() } catch (e: any) { failure('Delete failed', e.message) }
  }
  ...
  return (
    <Card>
      <CardHeader className="flex-row items-center justify-between space-y-0 py-3">
        ...
        <Button size="sm" onClick={() => { setModal({ type: 'space' }); setName('') }}><Plus className="size-3.5 mr-1" />Space</Button>
      </CardHeader>
      {err && <div className="px-4 pb-2"><p className="rounded-sm bg-destructive/10 px-3 py-2 text-xs text-destructive">{err}</p></div>}
      <div className="px-4 pb-4 space-y-3">
        {spaces.length === 0 && <p className="text-xs text-muted-foreground py-4 text-center">No spaces yet. Create one to group disks.</p>}
        {spaces.map(s => (
          <div key={s.id} className="rounded-sm border border-border">
            <div className="flex items-center justify-between bg-muted/30 px-3 py-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] text-muted-foreground">· {s.disks.length} disk{s.disks.length !== 1 ? 's' : ''}</span>
              </div>
              <div className="flex gap-1">
                <Button variant="ghost" size="sm" onClick={() => { setModal({ type: 'disk', spaceId: s.id }); setName(''); setPath('') }}><Plus className="size-3" /></Button>
                <Button variant="ghost" size="sm" onClick={() => { setModal({ type: 'space', id: s.id }); setName(s.name) }}>Rename</Button>
                <Button variant="ghost" size="sm" onClick={() => { setConfirmSpaceId(s.id); setConfirmSpaceName(s.name) }}><Trash2 className="size-3 text-destructive" /></Button>
              </div>
            </div>
            {s.disks.length > 0 ? (
              <table className="w-full text-sm">
                    ...
                    <tr key={d.id} className="border-b border-border/50 hover:bg-muted/50 transition-colors">
                      ...
                      <td className="py-1.5 pr-3">
                        <div className="flex justify-end gap-1">
                          ...
                          <Button variant="ghost" size="sm" onClick={() => { setModal({ type: 'disk', id: d.id }); setName(d.name); setPath(d.path) }}>Edit</Button>
                          <Button variant="ghost" size="sm" onClick={() => { setConfirmDiskId(d.id); setConfirmDiskName(d.name) }}><Trash2 className="size-3 text-destructive" /></Button>
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      <Dialog open={modal?.type === 'space'} onOpenChange={() => setModal(null)}>
        <DialogContent className="sm:max-w-[360px]">
          <DialogHeader><DialogTitle>{modal?.id ? 'Rename Space' : 'New Space'}</DialogTitle></DialogHeader>
          <form onSubmit={modal?.id ? renameSpace : addSpace} className="space-y-3">
            ...
            <Button type="submit" className="w-full" disabled={!name}>{modal?.id ? 'Rename' : 'Create'}</Button>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={modal?.type === 'disk'} onOpenChange={() => setModal(null)}>
        <DialogContent className="sm:max-w-[420px]">
          <DialogHeader><DialogTitle>{modal?.id ? 'Edit Disk' : 'Add Disk'}</DialogTitle></DialogHeader>
          <form onSubmit={modal?.id ? editDisk : addDisk} className="space-y-3">
            ...
            <Button type="submit" className="w-full" disabled={!name || !path}>{modal?.id ? 'Save' : 'Create'}</Button>
          </form>
        </DialogContent>
      </Dialog>
...
function TeamsDialog({ diskId, diskName, onClose, onChanged }: { diskId: number | null; diskName: string; onClose: () => void; onChanged: () => void }) {
  const [teams, setTeams] = useState<DiskTeam[]>([])
  ...
  const load = useCallback(async () => {
    if (!diskId) return
    setBusy(true)
    try { setTeams(await fetchDiskTeams(diskId)) } catch { /* */ } finally { setBusy(false) }
  }, [diskId])
  ...
  const addTeam = async (e: React.FormEvent) => {
    ...
    try { await createDiskTeam(diskId, newName.trim()); success('Team created', newName.trim()); setNewName(''); void load(); onChanged() } catch (e: any) { failure('Create failed', e.message) }
  }
  ...
  const saveTeam = async () => {
    ...
    try {
      ...
      void load()
      onChanged()
    } catch (e: any) { failure('Update failed', e.message) }
  }

  const delTeam = async (id: number) => {
    try { await deleteDiskTeam(id); success('Team deleted'); void load(); onChanged() } catch (e: any) { failure('Delete failed', e.message) }
  }

  const addUser = () => {
    if (!editing) return
    const names = userInput.split(/[,;\s]+/).map(s => s.trim()).filter(Boolean)
  ...
  return (
    <Dialog open={diskId !== null} onOpenChange={() => { if (!editing) onClose() }}>
      <DialogContent className="sm:max-w-[500px] max-h-[80vh] overflow-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Users className="size-4" />Teams — {diskName}
          </DialogTitle>
        </DialogHeader>
        ...
        {!editing && (
          <>
            <div className="space-y-2">
              {teams.map(t => (
                <div key={t.id} className="flex items-center justify-between rounded-md border border-border/50 px-3 py-2 text-sm">
                  ...
                  <div className="flex gap-1 shrink-0 ml-2">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => setConfirmId(t.id)}><Trash2 className="size-3 text-destructive" /></Button>
                  </div>
                </div>
              ))}
            </div>
            ...
            <form onSubmit={addTeam} className="flex gap-2 pt-2">
              <Input placeholder="New team name" value={newName} onChange={e => setNewName(e.target.value)} className="flex-1" />
        ...
        {editing && (
          <div className="space-y-3">
            <Input placeholder="Team name" value={editing.name} onChange={e => setEditing({ ...editing, name: e.target.value })}
              onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); saveTeam() } }} />

            <div>
              <p className="text-xs font-medium mb-1 text-muted-foreground">Users</p>
              <div className="flex flex-wrap gap-1 mb-2">
                {editing.users.length === 0 && <span className="text-[11px] text-muted-foreground italic">No users</span>}
                {editing.users.map(u => (
                  <span key={u} className="inline-flex items-center gap-1 rounded-sm bg-muted px-2 py-0.5 text-[11px]">
                    {u}
                    <button onClick={() => removeUser(u)} className="text-muted-foreground hover:text-foreground transition-colors">
              ...
              <div className="flex gap-2">
                <Input placeholder="Add users (comma separated)" value={userInput} onChange={e => setUserInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); addUser() } }} className="flex-1" />
                <Button size="sm" variant="outline" onClick={addUser} disabled={!userInput.trim()}>Add</Button>
              </div>
            </div>

            <div className="flex justify-end gap-2 pt-2">
              <Button variant="outline" size="sm" onClick={() => setEditing(null)}>Cancel</Button>
              <Button size="sm" onClick={saveTeam}>Save</Button>
            </div>
          </div>
        )}

        <ConfirmDialog open={confirmId !== null} onOpenChange={() => setConfirmId(null)} title="Delete Team" description="Remove this team and its user list? This cannot be undone." action="Delete" onConfirm={() => { if (confirmId !== null) delTeam(confirmId); setConfirmId(null) }} />
      </DialogContent>
    </Dialog>
  )
}
...
function ProfilePanel({ user }: { user: AuthInfo['user'] }) {
  ...
  const go = async (e: React.FormEvent) => {
    e.preventDefault()
    if (np.length < 10) { setMsg('Password must be at least 10 characters'); return }
    try { await changeOwnPassword(cp, np); success('Password changed'); setMsg('Password changed.'); setCp(''); setNp('') } catch (e: any) { setMsg(e.message); failure('Password change failed', e.message) }
  }
  if (!user) return null
  return (
    <Card>
      <CardHeader className="py-3"><CardTitle className="text-sm">Profile</CardTitle></CardHeader>
      <CardContent className="space-y-4">
        <div className="flex items-center gap-3">
          <div className="flex size-10 items-center justify-center rounded-full bg-muted"><User className="size-4 text-muted-foreground" /></div>
          <div>
            <p className="text-sm font-medium">{user.username}</p>
            <Badge variant={user.role === 'owner' ? 'default' : 'secondary'} className="text-[10px] mt-0.5">{user.role}</Badge>
          </div>
        </div>
        <Separator />
        <form onSubmit={go} className="space-y-3 max-w-sm">
          <h4 className="text-sm font-medium">Change Password</h4>
          {msg && <p className={`rounded-sm px-3 py-2 text-xs ${msg.includes('changed') ? 'bg-primary/10 text-primary' : 'bg-destructive/10 text-destructive'}`}>{msg}</p>}
          ...
          <Button type="submit" disabled={!cp || np.length < 10}>Change Password</Button>
        </form>
      </CardContent>
    </Card>
  )
...
export function AdminTab() {
  ...
  if (auth?.needsSetup) return <SetupForm onDone={u => setAuth({ loggedIn: true, user: u, needsSetup: false, rateLimit: { captcha: false, attempts: 0 } })} />
  if (!auth?.loggedIn) return <LoginForm onLogin={u => setAuth({ loggedIn: true, user: u, needsSetup: false, rateLimit: auth!.rateLimit })} rateLimit={auth!.rateLimit} />
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-4 animate-fade-in w-full">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <Button variant="ghost" size="sm" onClick={async () => { await logout(); success('Signed out') }}><LogOut className="size-3.5 mr-1" />Sign out</Button>
        </div>
      </div>
