...
export function AlertDialog({ open: controlled, onOpenChange, children }: { open: boolean; onOpenChange: (v: boolean) => void; children: ReactNode }) {
...
export function AlertDialogTrigger({ asChild, onClick, children, className, ...props }: any) {
  ...
  if (asChild) {
    return <span onClick={() => { onClick?.(); onOpenChange(true) }} {...props}>{children}</span>
  }
  return <button onClick={() => { onClick?.(); onOpenChange(true) }} className={className} {...props}>{children}</button>
}
...
export function AlertDialogContent({ children, className }: { children: ReactNode; className?: string }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in" onClick={() => onOpenChange(false)}>
      <div ref={ref} tabIndex={-1} className={cn('bg-card border border-border rounded-md shadow-lg max-w-md w-full mx-4 animate-slide-up', className)} onClick={e => e.stopPropagation()}>
        {children}
      </div>
    </div>
  )
...
export function AlertDialogAction({ onClick, children, className, ...props }: any) {
  const { onOpenChange } = useContext(Ctx)
  return <button onClick={() => { onClick?.(); onOpenChange(false) }} className={cn('inline-flex items-center rounded-sm bg-primary text-primary-foreground px-4 py-1.5 text-xs font-medium hover:opacity-90 transition-colors', className)} {...props}>{children}</button>
}

export function AlertDialogCancel({ onClick, children, className, ...props }: any) {
  const { onOpenChange } = useContext(Ctx)
  return <button onClick={() => { onClick?.(); onOpenChange(false) }} className={cn('inline-flex items-center rounded-sm border border-border px-4 py-1.5 text-xs hover:bg-muted transition-colors', className)} {...props}>{children}</button>
}
