...
function main(): void {
  switch (cmd) {
    case 'seed': {
      ...
      if (!user || !pass) {
        console.error('Usage: seed <username> <password>')
        process.exit(1)
      }
      if (hasAnyAdmin()) {
        console.error('Admin accounts already exist. Use reset-password to change password.')
        process.exit(1)
      }
    ...
    case 'list': {
      ...
      if (admins.length === 0) {
        console.log('No admin accounts. Run seed to create one.')
        break
      }
    ...
    case 'reset-password': {
      const [user, pass] = args
      if (!user || !pass) {
        console.error('Usage: reset-password <username> <password>')
        process.exit(1)
      }
    ...
    case 'rm': {
      const [user] = args
      if (!user) {
        console.error('Usage: rm <username>')
        process.exit(1)
      }
      const admin = getAdminByUsername(user)
      if (!admin) {
    ...
    case 'add-space': {
      const [name] = args
      if (!name) {
        console.error('Usage: add-space <name>')
        process.exit(1)
      }
    ...
    case 'add-disk': {
      ...
      if (!sid || !name || !path) {
        console.error('Usage: add-disk <space-id> <name> <path>')
        process.exit(1)
      }
